...
import { BIBLE_CHARACTERS, shuffleCharacters, type BibleCharacter } from "@/lib/bible-characters";
...
function ResultIcon({ phase }: { phase: Phase }) {
  ...
  return (
    ...
    >
      <div
        className={`animate-pop-in w-24 h-24 rounded-full flex items-center justify-center shadow-2xl ${
          isCorrect
            ? "bg-[#5B7A4A] text-white"
            : "bg-[#9C3A2A] text-white"
        }`}
      >
        {isCorrect ? (
...
export default function GuessTheCharacterPage() {
  ...
  useEffect(() => {
    setDeck(shuffleCharacters(BIBLE_CHARACTERS));
  }, []);
  ...
  const checkGuess = useCallback(() => {
    ...
    if (current.acceptedAnswers.includes(normalised)) {
      ...
      setShakeKey((k) => k + 1);
      playWrong();
      // Reset back to playing after the animation
      setTimeout(() => {
        setGuess("");
        setPhase("playing");
    ...
    return (
      <div className="min-h-screen flex flex-col bg-vellum">
        ...
        <div className="flex-1 flex flex-col items-center justify-center px-6 py-16 text-center">
          ...
          <h1 className="text-5xl font-bold text-ink mb-3" style={{ fontFamily: "var(--font-display)" }}>
            Well played!
          </h1>
          <p className="text-stone-mid text-lg mb-8">
            You got{" "}
            <span className="font-bold text-ink">{score} out of {total}</span> correct
            {bestStreak > 2 && (
              <> · best streak <span className="font-bold text-gold-deep">{bestStreak} 🔥</span></>
            )}
          </p>
  ...
  if (!current) {
    return (
      <div className="min-h-screen flex items-center justify-center bg-vellum">
        <p className="text-stone-light text-sm animate-pulse">Loading…</p>
      </div>
    );
  }
    ...
    <div className="min-h-screen flex flex-col bg-vellum">
      ...
      <div className="border-b border-stone-edge bg-parchment-soft">
        <div className="max-w-xl mx-auto px-4 py-3 flex items-center justify-between gap-4">
          <Link href="/games" className="text-stone-mid hover:text-ink transition-colors">
      ...
      <div className="flex-1 flex flex-col items-center justify-center px-6 py-10 max-w-xl mx-auto w-full">
        ...
        {/* Emoji clues + result icon overlay */}
        <div className="relative text-center mb-10 w-full">
          <p className="text-[10px] font-bold uppercase tracking-[0.35em] text-stone-light mb-6" style={{ fontFamily: "var(--font-accent)" }}>
            Who am I?
          </p>
          <div className={`flex items-center justify-center gap-3 sm:gap-5 flex-wrap transition-all duration-200 ${isWrong ? "opacity-30" : "opacity-100"}`}>
        ...
        {/* Answer card (correct / revealed) */}
        {isAnswered ? (
          <div className={`w-full rounded-3xl border-2 px-7 py-6 text-center mb-6 transition-all ${
            phase === "correct" ? "bg-[#f0f7eb] border-[#5B7A4A]/40" : "bg-parchment-soft border-stone-edge"
          }`}>
            {phase === "correct" && (
              <p className="text-[#5B7A4A] text-xs font-bold uppercase tracking-widest mb-2" style={{ fontFamily: "var(--font-accent)" }}>✓ Correct!</p>
            )}
            {phase === "revealed" && (
              <p className="text-stone-light text-xs font-bold uppercase tracking-widest mb-2" style={{ fontFamily: "var(--font-accent)" }}>The answer was</p>
            )}
              ...
              <input
                ...
                className={`flex-1 px-5 py-3.5 rounded-2xl border-2 text-ink placeholder:text-stone-light text-base focus:outline-none transition-all ${
                  isWrong
                    ...
                    : "border-stone-edge bg-white focus:border-gold focus:ring-1 focus:ring-gold"
                }`}
        ...
        {phase === "playing" && (
          <p className="mt-8 text-xs text-stone-light text-center italic max-w-xs" style={{ fontFamily: "var(--font-display)" }}>
            Each emoji is a clue. Think carefully — names from both Testaments count.
          </p>
        )}
